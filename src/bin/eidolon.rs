...
fn add_game (name:&str, exe: &str) {
    ...
    let entries = fs::read_dir("/home/nicohman/.config/eidolon/games").expect("Can't read in games");
    let mut can_be_used = true;
    for entry in entries {
        let base = entry.expect("unable to get entry").path().into_os_string().into_string().unwrap();
    ...
    if can_be_used == true {
        ...
        if res.is_ok() {
            ...
            let mut file = OpenOptions::new()
                ...
                .unwrap();
            file.write_all(String::from(String::from("#!/bin/bash\n")+&(path.into_os_string().into_string().unwrap())).as_bytes()).expect("Could not write game registry");
        }
    }
}
fn update_steam() {
    ...
    for entry in entries {
        ...
        if res.is_ok() {
            ...
            let mut file = OpenOptions::new()
                ...
                .unwrap();
            file.write_all((String::from("#!/bin/bash\nsteam 'steam://rungameid/")+&results.0+"'").as_bytes()).expect("Couldn't create game registration");
        } else {
...
fn search_games(rawname: String, steamdir:String) -> (String, String, String) {
    ...
    for entry in entries {
        ...
        if new_entry.find("appmanifest").is_some() {
            ...
            f.read_to_string( & mut contents).expect("Could not read appmanifest");
            unsafe {
                //Slices out the game data from the appmanifest. Will break the instant steam changes appmanifest formats
                let outname = contents.slice_unchecked(contents.find("installdir").unwrap() + 14, contents.find("LastUpdated").unwrap() - 4);
